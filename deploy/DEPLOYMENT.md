...
for the deep dive. This file records the **Flowers-specific** facts.

Flowers is a **purely static site** (a bundled `dist/`: `index.html` + `app.js`).
...
API 'new --name flowers'     # create the VM (returns https_url + proxy_port 8000)

deploy/provision.sh flowers  # /srv, Caddyfile, enable docker, run Caddy :8000
deploy/deploy.sh   flowers   # npm run build + rsync dist/ → /srv/site
```

VM SSH (`ssh exedev@flowers.exe.xyz`) works from anywhere; that's what the scripts use.
...
```sh
ssh -o ServerAliveInterval=5 -o ServerAliveCountMax=3 exe.dev share port flowers 8000
ssh -o ServerAliveInterval=5 -o ServerAliveCountMax=3 exe.dev share set-public flowers
```
...
`deploy/setup-autodeploy.sh flowers` installs an on-VM systemd timer that polls
...
Watch it: `ssh exedev@flowers.exe.xyz journalctl -u fish-autodeploy -f`.

## Verify

```sh
curl -sS -o /dev/null -w "%{http_code}\n" https://flowers.exe.xyz/                       # 200 when public
ssh exedev@flowers.exe.xyz 'curl -s -o /dev/null -w "%{http_code}\n" localhost:8000/'    # always 200 if Caddy is up
```

## Quick recovery

- **Down after reboot:** `ssh exedev@flowers.exe.xyz 'sudo systemctl enable --now docker; docker start fish'` (the container is named `fish`)
- **`Host key verification failed`:** VM was reprovisioned — `ssh-keygen -R flowers.exe.xyz` (scripts already pass `accept-new`).
- **Deploy/gateway SSH hangs:** use the HTTPS API for control-plane, VM SSH for the box.
- **VM gone:** `API 'new --name flowers'` → `provision.sh` → `deploy.sh` → re-run the public step. Everything is in this repo.
