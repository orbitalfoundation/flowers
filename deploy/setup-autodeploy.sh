...
#   deploy/setup-autodeploy.sh [vm-name]      (default: flowers)
#
...
SSH 'sudo tee /etc/systemd/system/flowers-autodeploy.service >/dev/null' <<'UNIT'
[Unit]
Description=Flowers — rebuild + redeploy when GitHub main changes
...
SSH 'sudo tee /etc/systemd/system/flowers-autodeploy.timer >/dev/null' <<'UNIT'
[Unit]
Description=Poll GitHub main for Flowers deploys
...
SSH 'sudo systemctl daemon-reload && sudo systemctl enable --now flowers-autodeploy.timer'

echo "› first run (deploys current main now)"
SSH 'sudo systemctl start flowers-autodeploy.service'
SSH 'systemctl status flowers-autodeploy.timer --no-pager -l | head -6'

echo "✓ auto-deploy installed. Watch it with:"
echo "    ssh $DEST journalctl -u flowers-autodeploy -f"
