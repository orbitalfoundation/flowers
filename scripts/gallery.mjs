...
const chrome = spawn(CHROME, ['--headless=new', `--remote-debugging-port=${PORT}`,
  ...
  '--window-size=900,900', '--no-first-run', '--user-data-dir=/tmp/flowergallery', 'about:blank']);
...
// exactly how an undeclared uniform once made every flower body invisible while
...
for (let i = 0; i < 40 && !ready; i++) {
  await sleep(500);
  ready = await evalJs(`typeof window.FLOWERS !== 'undefined' && !!window.FLOWERS.flower()`);
}
if (!ready) {
  console.error(`ERROR: ${URL} is not the flower app (no window.FLOWERS). Is another project's server on that port?`);
...
for (const id of list) {
  await evalJs(`window.FLOWERS.setSpecies(${JSON.stringify(id)}); true;`);
  await sleep(1400);
...
await evalJs(`window.FLOWERS.setSpecies('dragonflower'); true;`);
await sleep(1200);
